    ...
    <div class="jshop row-fluid">
   		<div class="span4"> 
            ...
            <?php if(!count($this->images)){?>
                <img id = "main_image" class="jshop_thumbnail" src = "<?php print $this->image_product_path?>/<?php print $this->noimage?>" alt = "<?php print htmlspecialchars($this->product->name)?>" data-zoom-image="<?php print $this->image_product_path?>/<?php print $image->image_full;?>" />
            <?php }?>
            ...
            <a class="lightbox" id="main_image_full_<?php print $image->image_id?>" href="<?php print $this->image_product_path?>/<?php print $image->image_full;?>" <?php if ($k!=0){?>style="display:none"<?php }?> title="<?php print htmlspecialchars($image->_title)?>">
                ...
                <div class="text_zoom">
                    <img style="float: none;" src="<?php print $this->path_to_image?>search.png" alt="zoom" /> <?php print _JSHOP_ZOOM_IMAGE?>
                </div>
            </a>
            <?php }?>
            </span>
   		...
   		<div class="span8">
   			<h1 class="vina_title text_color"><?php print $this->product->name?></h1>
			...
			<div class="review_rate">
				<div class="review_rate-top">
					...
					<?php if ($this->config->product_show_button_back){?>
						<div class="link_back">
							<input type="button" class="button" value="<?php print _JSHOP_BACK.' to: '.$category->name;?>" onclick="<?php print $this->product->button_back_js_click;?>" />				
						</div>
					<?php }?>
				</div>
				<div class="review_rate-bottom">
					...
					<div class="manufacturer_name">
						<span class="vina-brand vina-bold"><?php print JText::_('VINA_BRAND')?>:</span> <span><?php print $this->product->manufacturer_info->name?></span>
					</div>
					<?php }?>
					<div class="vina-code-prod">
						<?php if ($this->config->show_product_code){?> <span class="vina-bold"><?php print _JSHOP_EAN_PRODUCT ?>: </span><span id="product_code"><?php print $this->product->getEan();?></span><?php }?>
					</div>
					...
					<?php if ($product->product_quantity >0){ ?>
						<div class = "availability" id="availability"><span class="vina-bold"><?php print JText::_('VINA_AVAILABILITY').': '?></span><?php print JText::_('VINA_AVAILABILITY_STOCK')?></div>
					<?php }
						else { ?>
			        ...
			        <div class="jshop">
						...
						<?php foreach($this->attributes as $attribut){?>
							<tr class="attributes_title">
								<td class="attributes_name"><?php print $attribut->attr_name?>:</td><span class="attributes_description"><?php print $attribut->attr_description;?></span>				           
								<td id='block_attr_sel_<?php print $attribut->attr_id?>'>
								<span><?php print $attribut->selects;?></span>								
								</td>
							</tr>
						<?php }?>
						</table>
				...
				<div class="prod_cart">
					...
					<div class="buttons buttons-detail">
						<div class="add-cart-detail">
							<i style="display: inline-block;" class="icon-shopping-cart"></i><input type="submit" id="add-cart" class="button" value="<?php print _JSHOP_ADD_TO_CART?>" onclick="jQuery('#to').val('cart');" />
						</div>	                
						<?php if ($this->enable_wishlist){?>
							<div class="add-wishlist-detail">
								<i style="display: inline-block;" class="icon-heart"></i><input type="submit" id="add-wishlist" class="button" value="<?php print _JSHOP_ADD_TO_WISHLIST?>" onclick="jQuery('#to').val('wishlist');" />
							</div>
						<?php }?>
			...
			<div class="prod_social">
   			...
   			<div class="jshop_prod_description">
		    	<h4><?php echo JText::_('VINA_JSHOP_SHORT_DESCRIPTION');?></h4>
